...
% Calculate number of loops N(Unused), for this projet as large as possible 
...
mask = abs(bestplane*[data; ones(1,size(data,2))])/sqrt...
    (plane_fit(1)^2+plane_fit(2)^2+plane_fit(3)^2) < sigma; % Choose inlier      
inliers_index = find(mask == 1);    % Find points
