...
IMAGENET = 'C:\Ethan\SiamFC\Pytorch-SiamFC-master\ILSVRC2017_VID\ILSVRC'
DEFAULT_MODEL =   'C:\Ethan\SiamFC\Pytorch-SiamFC-master\BaselinePretrained.pth.tar'

def parse_arguments():
    parser = argparse.ArgumentParser(description="SiamFC app")
    parser.add_argument('-f', '--fps', default=30, dest="fps", type=int,
    ...
    parser.add_argument('-s', '--seq', default=21, dest="seq", type=int,
                        help="The number of the sequence to be displayed."
                             "according to alphabetic order")
    parser.add_argument('-t', '--type', default='val', choices=['train', 'val','test'],
    ...
    parser.add_argument('-c', '--ctx_mode', default='mean', choices=['max', 'mean'],
...
if __name__ == '__main__':
    ...
    display = MainUI(win, BUFFER, disp_prior=args.prior_width,exit_on_end=args.exit_on_end, alpha=args.alpha)
